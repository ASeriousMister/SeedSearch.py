...
def btc_list_der(lang, how_many):
    ...
    while index < how_many:
        btc_list.append(
            add_der(BTC, lang, checking_str, '', 44, 0, index, False))
        index += 1
    # bitcoin bip44 hardened addresses
    index = 0
    while index < how_many:
        btc_list.append(
            add_der(BTC, lang, checking_str, '', 44, 0, index, True))
        index += 1
    # bitcoin bip49 not hardened addresses
    index = 0
    while index < how_many:
        btc_list.append(
            add_der(BTC, lang, checking_str, '', 49, 0, index, False))
        index += 1
    # bitcoin bip49 hardened addresses
    index = 0
    while index < how_many:
        btc_list.append(
            add_der(BTC, lang, checking_str, '', 49, 0, index, True))
        index += 1
    # bitcoin bip84 not hardened addresses
    index = 0
    while index < how_many:
        btc_list.append(
            add_der(BTC, lang, checking_str, '', 84, 0, index, False))
        index += 1
    # bitcoin bip84 hardened addresses
    index = 0
    while index < how_many:
        btc_list.append(
            add_der(BTC, lang, checking_str, '', 84, 0, index, True))
        index += 1
    # sam premix= 2147483645' postmix= 2147483646'
    index = 0
    while index < how_many:
...
def ltc_list_der(lang, how_many):
    ...
    while index < how_many:
        ltc_list.append(
            add_der(LTC, lang, checking_str, '', 44, 0, index, False))
        index += 1
    # litecoin bip44 hardened addresses
    index = 0
    while index < how_many:
        ltc_list.append(
            add_der(LTC, lang, checking_str, '', 44, 0, index, True))
        index += 1
    # litecoin bip49 not hardened addresses
    index = 0
    while index < how_many:
        ltc_list.append(
            add_der(LTC, lang, checking_str, '', 49, 0, index, False))
        index += 1
    # litecoin bip49 hardened addresses
    index = 0
    while index < how_many:
        ltc_list.append(
            add_der(LTC, lang, checking_str, '', 49, 0, index, True))
        index += 1
    # litecoin bip84 not hardened addresses
    index = 0
    while index < how_many:
        ltc_list.append(
            add_der(LTC, lang, checking_str, '', 84, 0, index, False))
        index += 1
    # litecoin bip84 hardened addresses
    index = 0
    while index < how_many:
        ltc_list.append(
            add_der(LTC, lang, checking_str, '', 84, 0, index, True))
        index += 1
    return ltc_list
...
if (len(seed2check) > 0):
    ans = input(color.BLUE + '\nDo you also want to see the addresses that could be derived with the found seeds? (y/n)\n' + color.END)
    if ((ans == 'y') or (ans == 'Y')):
        conn = is_connected()
        if conn:
            ans2 = input(color.BLUE + 'Do you want to query public APIs to try to find out how the seeds were used? (y/n)\n' + color.END)
            if ((ans2 == 'y') or (ans2 == 'Y')):
                online_check = True
            else:
                online_check = False
        ...
        prog = 0
        how_many = input(color.BLUE + 'How many addresses do you want seedsearch to derive for each derivation path (max = 10)? \n' + color.END)
        ...
        while prog < len(seed2check):
            ...
            if online_check == False:
                ...
                i = 0
                while i < len(btc_list):
                    link = 'https://blockchain.info/q/addressfirstseen/' + \
                        btc_list[i]
                ...
                i = 0
                while i < len(eth_list):
                    link = 'https://api.blockcypher.com/v1/eth/main/addrs/' + \
                        eth_list[i]
                    time.sleep(400/1000)   # avoid blockcypher's limit of 3 calls per second
                ...
                i = 0
                while i < len(ltc_list):
                    ltc_tx = blockcypher.get_total_num_transactions(
                        ltc_list[i], coin_symbol='ltc')
                    time.sleep(400/1000)   # avoid blockcypher's limit of 3 calls per second
                    if ltc_tx != 0:
                        if i < how_many:
            ...
            #            i = 6
                ...
                i = 0
                while i < len(zec_list):
                    link = 'https://api.zcha.in/v2/mainnet/accounts/' + \
                        zec_list[i]
